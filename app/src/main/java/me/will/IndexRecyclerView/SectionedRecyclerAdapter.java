...
public class SectionedRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private static final int SECTION_TYPE_TITLE = 0;
    private static final int SECTION_TYPE_HEADER = -1;
    private static final int SECTION_SHOW_LIST = 2;
    ...
    public static class HeaderViewHolder extends RecyclerView.ViewHolder {
        ...
        public HeaderViewHolder(View view, int resourceId) {
            super(view);
            title = (TextView) view.findViewById(resourceId);
        }
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        Log.e("recylerview", "viewType;;;;;" + viewType);
        if (viewType == SECTION_TYPE_HEADER) {
            final View view = mLayoutInflater.inflate(R.layout.item_header_view, parent, false);
            return new HeaderViewHolder(view, R.layout.item_header_view);
        } else if (viewType == SECTION_TYPE_TITLE) {
...
//        Log.e("recylerview", "position:" + position);
        if (position == 0) {

        } else if (isSectionHeaderPosition(position)) {
            ((SectionViewHolder) sectionViewHolder).title.setText(mSections.get(position).title);
            if (mSections.get(position).title.toString().contains("-")) {
                ((SectionViewHolder) sectionViewHolder).title.setText("历史城市");
            } else if (mSections.get(position).title.toString().contains("+")) {
                ((SectionViewHolder) sectionViewHolder).title.setText("已定位城市");
            } else if (mSections.get(position).title.toString().contains("=")) {
                ((SectionViewHolder) sectionViewHolder).title.setText("热门城市");
            } else {
                ((SectionViewHolder) sectionViewHolder).title.setText(mSections.get(position).title);
            }
        } else {
//            Log.e("recylerview", "position:" + position+"   ");
    ...
    public int getItemViewType(int position) {
        return position == 0 ? SECTION_TYPE_HEADER : (isSectionHeaderPosition(position)
                ? SECTION_TYPE_TITLE
